const { fetchData } = require("./main.js");

describe("main-fetchData", () => {
  test('given all params but source id should return missing source.id param"', () => {
    expect(
      fetchData({
  ...
  test('given empty object should return missing all params"', () => {
  ...
  test('given no args should return missing all params"', () => {
    expect(fetchData({})).rejects.toThrow(
      "widget did not recieve all needed params, missing: publisher id, app.type, app.apikey, source.id, source.type, count"
    );
  });
